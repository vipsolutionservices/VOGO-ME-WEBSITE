...
function buildOfferDetailLayout(offer, offerIndex) {
  ...
  const row1RightMarkup = row1RightImage
    ? `<button class="support-gallery-thumb support-gallery-thumb--row1" type="button" data-gallery-index="${offer.supportGalleryImages.indexOf(row1RightImage)}" aria-label="Open image ${preferredImageNames.row1Right}">
        <img src="${row1RightImage}" alt="VOGO AI preview - main image" loading="lazy" decoding="async" />
      </button>`
    : '';
  ...
  const row2LeftMarkup = row2LeftImage
    ? `<button class="support-gallery-thumb support-gallery-thumb--row2" type="button" data-gallery-index="${offer.supportGalleryImages.indexOf(row2LeftImage)}" aria-label="Open image ${preferredImageNames.row2Left}">
        <img src="${row2LeftImage}" alt="VOGO AI preview - secondary image" loading="lazy" decoding="async" />
      </button>`
    : '';
  ...
  const row3WideMarkup = row3WideImage
    ? `<button class="support-gallery-thumb support-gallery-thumb--wide" type="button" data-gallery-index="${offer.supportGalleryImages.indexOf(row3WideImage)}" aria-label="Open image ${preferredImageNames.row3Wide}">
        <img src="${row3WideImage}" alt="VOGO AI preview - panoramic image" loading="lazy" decoding="async" />
      </button>`
    : '';
  ...
  return `
    <div class="support-detail-layout support-detail-table-layout">
      ...
      <div class="support-detail-table-row support-detail-table-row--4">
        <div class="support-detail-actions" aria-label="Support actions">
...
(function initEnterpriseContactForm() {
  ...
  function validateForm() {
    ...
    if (emailValue && !emailIsValid) {
      setFieldError(fields.email, 'Email address is invalid.');
      valid = false;
    }

    const phoneValue = (fields.phone.input?.value || '').trim();
    if (phoneValue && !/^[0-9+()\-.\s]{6,}$/.test(phoneValue)) {
      setFieldError(fields.phone, 'Phone number is invalid.');
      valid = false;
    }

    const projectText = (fields.project.input?.value || '').trim();
    if (projectText && projectText.length < 20) {
      setFieldError(fields.project, 'Project description must be at least 20 characters long.');
      valid = false;
    }

    if (String(captchaInputEl?.value || '').trim() !== String(captchaAnswer)) {
      showCaptchaError('Incorrect captcha. Try again.');
      valid = false;
    }
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (response.ok && json?.success) {
        setStatus('Your request was sent successfully. Check your business email for confirmation.', 'success');
        form.reset();
        regenerateCaptcha();
      ...
      const errorMessage =
        ...
        `We could not send the request right now (code ${response.status || 'unknown'}). Please try again.`;
